...
export default function Profile() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 flex flex-col gap-4">
      ...
      <div className="rounded-2xl p-4 bg-(--gc-surface) flex flex-col gap-3" style={{ border: "1px solid var(--gc-border)" }}>
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs truncate" style={{ color: "var(--gc-muted)" }}>{currentUser.email}</p>
          </div>
        </div>

        <label className="flex flex-col gap-1 text-xs">
          <span style={{ color: "var(--gc-muted)", fontWeight: 700 }}>{t.name}</span>
          <input
            value={name}
        ...
        <button
          ...
          style={{ background: canSave ? "var(--gc-forest)" : "var(--gc-border)", color: canSave ? "#fff" : "var(--gc-muted-light)", cursor: canSave ? "pointer" : "not-allowed" }}
        >
          {t.admin.save}
        </button>
      </div>

      <div className="rounded-2xl p-4 bg-(--gc-surface) flex items-center justify-between" style={{ border: "1px solid var(--gc-border)" }}>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Link
          to="/sevimlilar"
          className="rounded-2xl p-4 bg-(--gc-surface) flex items-center gap-2.5 transition hover:-translate-y-0.5"
      ...
      <div className="rounded-2xl p-4 bg-(--gc-surface) flex flex-col gap-3" style={{ border: "1px solid var(--gc-border)" }}>
        ...
        {savedAddresses.length === 0 ? (
          <p className="text-sm" style={{ color: "var(--gc-muted)" }}>{t.noSavedAddresses}</p>
        ) : (
            ...
            {savedAddresses.map((a) => (
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="flex items-start gap-1 text-[11px] mt-0.5" style={{ color: "var(--gc-muted-dark)" }}>
      ...
      <button
        ...
        className="flex items-center justify-center gap-2 rounded-2xl p-4 bg-(--gc-surface) text-sm font-bold transition hover:bg-black/2"
